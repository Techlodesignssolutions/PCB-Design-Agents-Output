...
def _strip_fp_text_blocks_non_cu(kicad_mod_text: str) -> str:
    out = []
    last = 0
    for m in re.finditer(r'\(fp_text\b[\s\S]*?\)\s*\)', kicad_mod_text):
        block = m.group(0)
        if '(layer "F.Cu")' in block or '(layer "B.Cu")' in block:
            continue
        out.append(kicad_mod_text[last:m.start()])
        last = m.end()
    out.append(kicad_mod_text[last:])
    return ''.join(out)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f'KiCadPart failed for {lib_or_path} (path: {path}) even after normalization/strip')
