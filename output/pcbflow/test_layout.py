...
def _assert_pads(part, ref):
    ...
    if n == 0:
        raise RuntimeError(f'{ref} parsed with 0 pads — footprint not supported')
    print(f'{ref}: {n} pad(s)')
    return part
...
brd = Board((50, 20))
# Resolve a valid top copper layer name for this pcbflow version
TOP = brd.get_copper_layers(as_names=True)[0]
MASK_TOP = brd.get_mask_layer(side='top', as_name=True)
PASTE_TOP = brd.get_paste_layer(side='top', as_name=True)

# Pad-point cache computed from JSON footprints (absolute DC coords)
PADPTS = {}

def pad_points_from_fp(key: str, origin: tuple[float, float]):
    spec = load_footprint_spec(key)
    ox, oy = origin
    pts = []
    for p in spec.get('pads', []):
        atx, aty = p.get('at', [0.0, 0.0])
        pts.append(brd.DC((ox + atx, oy + aty)))
    return pts

def draw_pad_geometry(key: str, origin: tuple[float, float], ref: str):
    spec = load_footprint_spec(key)
    ox, oy = origin
    for p in spec.get('pads', []):
        atx, aty = p.get('at', [0.0, 0.0])
        ...
        if spec.get('type') == 'smt':
            sx, sy = p.get('size', [1.0, 1.0])
            # Build rectangle corners from center and size
            tl = (cx - sx/2, cy - sy/2)
            br = (cx + sx/2, cy + sy/2)
            brd.add_named_rect(tl, br, layer=TOP, name=f'{ref}_pad')
            brd.add_named_rect(tl, br, layer=MASK_TOP, name=f'{ref}_mask')
            brd.add_named_rect(tl, br, layer=PASTE_TOP, name=f'{ref}_paste')
        else:
            dia = p.get('diameter', 2.0)
            drill = p.get('drill', 1.0)
            # Annulus as square for simplicity
            tl = (cx - dia/2, cy - dia/2)
            br = (cx + dia/2, cy + dia/2)
            brd.add_named_rect(tl, br, layer=TOP, name=f'{ref}_tht')
            brd.add_named_rect(tl, br, layer=MASK_TOP, name=f'{ref}_mask')
            brd.add_drill((cx, cy), drill)
...
# Place J1 connector
J1_ORIGIN = (25.0, 4.0)
J1 = safe_build_fp(brd.DC(J1_ORIGIN), 'JST_2_THT', side='top')
PADPTS['J1'] = pad_points_from_fp('JST_2_THT', J1_ORIGIN)
draw_pad_geometry('JST_2_THT', J1_ORIGIN, 'J1')

# Place D1 LED
D1_ORIGIN = (25.0, 15.0)
D1 = safe_build_fp(brd.DC(D1_ORIGIN), 'LED_0805', side='top')
PADPTS['D1'] = pad_points_from_fp('LED_0805', D1_ORIGIN)
draw_pad_geometry('LED_0805', D1_ORIGIN, 'D1')

# Place R1 resistor near D1
R1_ORIGIN = (25.0, 10.0)
R1 = safe_build_fp(brd.DC(R1_ORIGIN), 'R_0805', side='top')
PADPTS['R1'] = pad_points_from_fp('R_0805', R1_ORIGIN)
draw_pad_geometry('R_0805', R1_ORIGIN, 'R1')
...
_a = PADPTS['J1'][0]
_b = PADPTS['R1'][0]
# Explicit copper rectangle trace between points
_x1, _y1 = (_a.at if hasattr(_a, 'at') else _a)
_x2, _y2 = (_b.at if hasattr(_b, 'at') else _b)
if _x1 == _x2:
    tl = (_x1 - 0.25/2, min(_y1, _y2))
    br = (_x2 + 0.25/2, max(_y1, _y2))
else:
    m = (_y2 - _y1) / (_x2 - _x1)
    # Approximate with bounding box for simplicity
    tl = (min(_x1, _x2), min(_y1, _y2))
    br = (max(_x1, _x2), max(_y1, _y2))
brd.add_named_rect(tl, br, layer=TOP, name='trace')
...
_x2, _y2 = (_b.at if hasattr(_b, 'at') else _b)
if _x1 == _x2:
    tl = (_x1 - 0.152/2, min(_y1, _y2))
    br = (_x2 + 0.152/2, max(_y1, _y2))
else:
    m = (_y2 - _y1) / (_x2 - _x1)
    # Approximate with bounding box for simplicity
    tl = (min(_x1, _x2), min(_y1, _y2))
